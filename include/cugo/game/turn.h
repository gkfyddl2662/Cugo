...
inline constexpr std::uint16_t kAllMonthBits = (std::uint16_t{1} << core::kMonthCount) - 1;
...
CUGO_HOST_DEVICE inline TurnState48 make_turn_state(const InitialDeal48& deal,
...
CUGO_HOST_DEVICE inline bool is_valid_turn_state(const TurnState48& state) noexcept {
  if (state.actor > 1u ||
      static_cast<std::uint8_t>(state.phase) > static_cast<std::uint8_t>(TurnPhase::kResolve) ||
...
CUGO_HOST_DEVICE inline TurnState48 load_turn_state(const TurnStateSoA48& states,
