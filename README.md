...
- fixed CPU examples plus randomized card-partition invariants
- CUDA CPU/GPU differential validation over 65,536 deals, stock transitions, and turn resolves
...
The measured deal and draw kernels use zero local memory and reach 100% theoretical occupancy. Longer block-size measurements showed 256 and 512 threads/block effectively tied, so 256 remains the working default to preserve headroom as the resolver grows.
...
Hangame's official guide is the source of truth. The pinned facts, implemented subset, and intentionally deferred behavior are documented in `docs/rules.md`.
...
Pi stealing is also not applied yet. The resolver emits the relevant event/ppuk-capture metadata, but exact pi transfer waits for card-category metadata so a legal pi card can be selected.
...
If CMake cannot find a CUDA compiler, it still builds and runs the CPU core test and prints that CUDA validation targets are disabled.
